...
draw s = [conc, health, ball, trainer, variable, botDialog]
  where
    botDialog = case s ^. phase of
      ChooseMove -> moveDialog
      Animation _ _ -> textDialog " "
      EndTurn Reduce False ->
        textDialog $ "Reduction brought the variables health down by " ++ show (reductionDmg s)
      EndTurn Reduce True ->
        textDialog "Great! Reduction got rid of the variable! Let's tame another one."
      EndTurn Rename _ ->
        textDialog $ "Renaming raised the damage multiplier to " ++ show (s ^. expr . damageMultiplier)
      EndTurn Coffee _ ->
        textDialog "Ahh... the coffee got you psyched again. Who needs to sleep anyway?"
    ...
    layoutDims = centerLayer
               . hLimit (fst dimensions)
               . vLimit (snd dimensions)

    layoutDialog = layoutDims . marginTop Max

    moveDialog = layoutDialog
               . renderDialog (s ^. nextMove)
               . hCenter
               . padAll 1
               . str $ "Choose a move:"
    ...
    health = layoutGraphics
           . marginBottom Max
           . marginRight Max
           . (<=> str ("health: " ++ show (s^.expr.curHealth) ++ "/" ++ show (s^.expr.maxHealth)))
           . hLimit 20
           $ progressBar Nothing (s^.expr.curHealth.to intToFloat / s ^.expr.maxHealth.to intToFloat)

    trainer = layoutGraphics
            . marginTop Max
            . marginRight Max
            . marginLeft (Pad 2)
            $ trainerSprite

    ball = case s^.phase of
      Animation Capture n ->
        let
          interp = min 1 . (* 1.5) $ (intToDouble n / intToDouble animationFrames)
          start = (24, 20)
          end = (56, 5)
          position = (round $ interp * fst end + (1-interp) * fst start, round $ interp* snd end + (1-interp)* snd start)
        in
          layoutGraphics
          . marginRight Max
          . marginBottom Max
    ...
    variable =
      layoutGraphics . marginBottom Max . marginLeft Max . marginRight (Pad 4) . marginTop (Pad 2) . varSprite . animateVar (s^.phase) $ (s^.expr.varChar)

    textDialog = layoutDialog
               . renderDialog (dialog Nothing Nothing (fst dimensions))
               . hCenter
               . padBottom (Pad 1)
               . padAll 1
               . str
...
topLeftCorner child =
  Widget Greedy Greedy $ do
    ...
    return $ childResult & imageL %~ resize (ctx^.availWidthL) (ctx^.availHeightL)
...
bottomRightCorner child =
  Widget Greedy Greedy $ do
    ...
    return $ addResultOffset offset $ childResult & imageL %~ translate offsetX offsetY
...
handleEvent :: AppState -> MyEvent -> EventM () (Next AppState)
handleEvent s (VtyEvent (EvKey KEsc _)) = halt s
handleEvent s (VtyEvent (EvKey KEnter _)) = case s ^. phase of
  ...
  EndTurn mv _ ->
    if s ^. concentration <= 1
      then halt s
        ...
        & phase .~ ChooseMove
        & concentration %~ subtract 1
        & case mv of
            Coffee -> remainingCoffee %~ subtract 1
            _ -> id
  ChooseMove -> continue $ s
    & if move == Coffee && s ^. remainingCoffee <= 0
        then id
        else phase .~ Animation move 0
    where Just move = s ^. nextMove . to dialogSelection
handleEvent s (VtyEvent ev) = case s ^. phase of
  ChooseMove -> continue =<< handleEventLensed s nextMove handleDialogEvent ev
  _ -> continue s
handleEvent s Tick = case s ^. phase of
  Animation Coffee n ->
    if s ^. concentration == 10
      then continue $ s
        & phase .~ EndTurn Coffee True
      else continue $ s
        & concentration %~ (+1)
        & phase .~ Animation Coffee (n+1)
  Animation Capture n ->
    if n >= animationFrames
      then do
        newExpr <- newExprWhenCaptured s
        continue $ s
          & maybe id (\e -> (expr .~ e) . (score %~ (+2))) newExpr
          & phase .~ EndTurn Capture (isJust newExpr)
      else
        continue (s & phase .~ Animation Capture (n+1))
  Animation Reduce n -> do
    ...
    if n >= animationFrames || dead
      then do
        ...
        continue $ s
          & calculateReduction n
          & phase .~ Animation Reduce (n+1)
  Animation Rename n ->
    if n >= animationFrames
      then continue $ s
        & expr . damageMultiplier %~ (+1)
        & expr . varChar %~ nextVar
        & phase .~ EndTurn Rename True
...
calculateReduction n s = s & expr . curHealth %~ hurt
  where
    ...
    hurt :: Int -> Int
    hurt hp = max 0 (hp + (n * totalDmg `div` animationFrames) - ((n+1) * totalDmg `div` animationFrames))
...
main = do
  ...
  forkIO $ forever $ do
    threadDelay 100000
    writeChan chan Tick
  s <- customMain (mkVty def) chan app initialState
